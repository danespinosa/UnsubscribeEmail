...
6. **Search (optional)**: The LLM calls `search_emails` with at least one filter for a capped, newest-first metadata-only result. Each invocation scans at most 100 candidate messages, reports `hasMore` when more candidates remain, returns exact message IDs, and does not issue N+1 attachment requests.
...
All MCP Graph clients request `Prefer: IdType="ImmutableId"` so message IDs remain stable across moves; the JSON values remain exact and each URL path segment is escaped once. Attachment listing returns Graph base metadata plus file-attachment `contentId` through a derived-type OData select, and never selects or includes other subtype-only fields or `contentBytes`. Downloaded content is bounded (4,000,000 bytes by default and 10,000,000 bytes maximum) and returned in the `base64Content` field with the response MIME type and inline/content ID metadata. Graph's metadata `size` is advisory, and the actual `downloadedSize` from `/$value` can differ because they describe different representations; differing values do not imply truncation when the bounded stream completes. The response `Content-Length` and stream enforce `maxBytes`, and a stream shorter than its declared length is reported as incomplete. File and item attachments use Graph's `/$value` endpoint; reference attachments are reported by type but rejected for download because they expose a URL rather than message content. Transient attachment failures return structured `statusCode`, `code`, `retryable`, `retryAfterSeconds`, `totalRetryDelaySeconds`, and `retryCount` fields in addition to the existing message. `get_email_content` retains its legacy `Id` property and also returns additive camel-case `messageId`; `read_emails` returns the exact `sampleMessageId` for the message represented by each sample body.
